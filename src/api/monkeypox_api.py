...
DateRequest = Annotated[str, Field(description="The date to search for in the format YYYY-MM-DD", pattern="\d{4}-\d{2}-\d{2}")]
...
async def latest_data() -> Records:
    ...
    return Records(date=date, records={k: v.to_dict(orient="records") for k, v in enumerate(dfs)})
...
async def search_by_date(date: DateRequest) -> Records:
    ...
    if report:
        ...
        return Records(date=report.date, records={k: v.to_dict(orient="records") for k, v in enumerate(dfs)})
    else:
        raise HTTPException(status_code=404, detail="No records exist for specified date")

@app.get("/data/{start_date}/{end_date}")
async def search_range(start_date: DateRequest, end_date: DateRequest) -> RecordsResponse:
